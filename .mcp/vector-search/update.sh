...
echo "🔍 Vector Search Update Started: $(date)" | tee -a "$LOG_FILE"
...
start_server() {
    echo "📡 Starting vector-search HTTP server..." | tee -a "$LOG_FILE"
    ...
    for i in {1..30}; do
        if check_server; then
            echo "✅ HTTP server started on port $VECTOR_SEARCH_PORT" | tee -a "$LOG_FILE"
            return 0
        fi
        sleep 1
    done
    echo "❌ HTTP server failed to start" | tee -a "$LOG_FILE"
    return 1
}
...
echo "📊 Indexing codebase: $REPO_ROOT" | tee -a "$LOG_FILE"
...
if echo "$RESPONSE" | grep -q '"success":true'; then
    ...
    echo "✅ Vector search indexed: $FILES_INDEXED files, $CHUNKS_CREATED chunks" | tee -a "$LOG_FILE"
else
    ERROR=$(echo "$RESPONSE" | grep -o '"error":"[^"]*"' | cut -d: -f2 | tr -d '"')
    echo "❌ Vector search update failed: $ERROR" | tee -a "$LOG_FILE"
    echo "Response: $RESPONSE" >> "$LOG_FILE"
fi

echo "✅ Vector Search Update Complete: $(date)" | tee -a "$LOG_FILE"
