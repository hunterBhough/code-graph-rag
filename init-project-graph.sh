...
if [ -z "$PROJECT_PATH" ]; then
    ...
    echo "  ✓ Create update script in .codebase-intelligence/code-graph/"
...
# Create .codebase-intelligence directory structure
log "Creating .codebase-intelligence/code-graph directory..."
mkdir -p "$PROJECT_PATH/.codebase-intelligence/code-graph/logs"
mkdir -p "$LOGS_DIR"
...
if ! grep -q "^\.codebase-intelligence/" "$GITIGNORE" 2>/dev/null; then
    echo "" >> "$GITIGNORE"
    echo "# Codebase intelligence - all auto-generated/derived data" >> "$GITIGNORE"
    echo ".codebase-intelligence/" >> "$GITIGNORE"
    log_success "Added .codebase-intelligence/ to .gitignore"
else
...
log_success "Created code-graph/update.sh"

# Create code-graph ignore file
if [ ! -f "$PROJECT_PATH/.codebase-intelligence/code-graph/ignore" ]; then
    log "Creating code-graph ignore file..."
    cat > "$PROJECT_PATH/.codebase-intelligence/code-graph/ignore" << 'IGNORE'
...
CODE_GRAPH_SCRIPT="$(git rev-parse --show-toplevel)/.codebase-intelligence/code-graph/update.sh"
...
# Update code-graph (background)
CODE_GRAPH_SCRIPT="$REPO_ROOT/.codebase-intelligence/code-graph/update.sh"
if [ -f "$CODE_GRAPH_SCRIPT" ]; then
    "$CODE_GRAPH_SCRIPT" &
    echo "✅ Code-graph update started in background"
fi
...
echo ""
log "Building graph... (monitor progress: tail -f $PROJECT_PATH/.codebase-intelligence/code-graph/update.log)"
...
echo ""
echo "📦 Installed:"
echo "  ✅ .codebase-intelligence/code-graph/ (update script, logs)"
if [ "$NO_HOOK" = false ]; then
...
echo "  View logs:     tail -f $PROJECT_PATH/.codebase-intelligence/code-graph/update.log"
echo ""
