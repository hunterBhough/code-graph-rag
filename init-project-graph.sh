#!/bin/bash
# Initialize weavr for a project
...
if [ -z "$PROJECT_PATH" ]; then
    ...
    echo "Sets up weavr for a project"
    echo ""
    ...
    echo "  ✓ Initialize code-graph database for the project"
    echo "  ✓ Create update script in .mcp/codebase-intelligence/weavr/"
    echo "  ✓ Install post-commit hook (unless --no-hook)"
    echo "  ✓ Build initial code graph"
    echo "  ✓ Register project in code-graph registry"
    echo ""
    exit 1
fi
...
print_header "🔗 Initializing Code-Graph for: $PROJECT_NAME"
...
log "Creating .mcp/codebase-intelligence/weavr directory..."
mkdir -p "$PROJECT_PATH/.mcp/codebase-intelligence/weavr/logs"
mkdir -p "$LOGS_DIR"
...
cat > "$PROJECT_PATH/.mcp/codebase-intelligence/weavr/update.sh" << SCRIPT_START
#!/bin/bash
...
PROJECT_LOG_DIR="\$REPO_ROOT/.mcp/codebase-intelligence/weavr/logs"
LOCAL_LOG="\$REPO_ROOT/.mcp/codebase-intelligence/weavr/update.log"
...
update_graph() {
    ...
    (
        ...
        if [ \$? -eq 0 ]; then
            echo "✅ \$db_name updated successfully" | tee -a "\$LOCAL_LOG"
        else
            echo "❌ \$db_name update failed (check logs)" | tee -a "\$LOCAL_LOG"
        fi
    )
}
...
create_log_symlinks() {
    if [ -n "\$GROUP" ]; then
        ...
        if [ ! -e "\$group_log" ] && [ -f "\$group_target" ]; then
            ...
            echo "🔗 Created symlink: \${GROUP}.log → centralized"
        fi
    fi
}

# Main execution
echo "🔄 Code-Graph Update Started: \$(date)"
...
echo "✅ Code-Graph Update Complete: \$(date)" | tee -a "\$LOCAL_LOG"
SCRIPT_START

chmod +x "$PROJECT_PATH/.mcp/codebase-intelligence/weavr/update.sh"
log_success "Created weavr/update.sh"

# Create code-graph ignore file
if [ ! -f "$PROJECT_PATH/.mcp/codebase-intelligence/weavr/ignore" ]; then
    log "Creating code-graph ignore file..."
    cat > "$PROJECT_PATH/.mcp/codebase-intelligence/weavr/ignore" << 'IGNORE'
# Project-specific exclusions for code-graph
# Global exclusions are handled by weavr
...
CODE_GRAPH_SCRIPT="$(git rev-parse --show-toplevel)/.mcp/codebase-intelligence/weavr/update.sh"
if [ -f "$CODE_GRAPH_SCRIPT" ]; then
    "$CODE_GRAPH_SCRIPT" &
    echo "✅ Code-graph update started in background"
fi
HOOK_APPEND
...
# Update code-graph (background)
CODE_GRAPH_SCRIPT="$REPO_ROOT/.mcp/codebase-intelligence/weavr/update.sh"
if [ -f "$CODE_GRAPH_SCRIPT" ]; then
    "$CODE_GRAPH_SCRIPT" &
    echo "✅ Code-graph update started in background"
fi
...
echo ""
log "Building graph... (monitor progress: tail -f $PROJECT_PATH/.mcp/codebase-intelligence/weavr/update.log)"
"$PROJECT_PATH/.mcp/codebase-intelligence/weavr/update.sh" || {
...
# Projects initialized with weavr
projects:
	# Example entry:
	# - name: my-project
...
# Step 7: Summary
#
print_header "✨ Code-Graph Setup Complete! ✨"
...
echo ""
echo "📦 Installed:"
echo "  ✅ .mcp/codebase-intelligence/weavr/ (update script, logs)"
if [ "$NO_HOOK" = false ]; then
    echo "  ✅ Post-commit hook"
fi
echo ""
echo "📝 Commands:"
echo "  Manual update: $PROJECT_PATH/.mcp/codebase-intelligence/weavr/update.sh"
echo "  View graph:    http://localhost:3000 (Memgraph Lab)"
echo "  View logs:     tail -f $PROJECT_PATH/.mcp/codebase-intelligence/weavr/update.log"
echo ""
